...
export function KindergartenReferenceLayout({config,onLayoutChange}:Props) {
 ...
 const classes=["Minik Kaşifler Sınıfı","Meraklı Bilim İnsanları","Renkli Düşler Sınıfı","Büyük Kaşifler Sınıfı"];
 ...
 return <main className="kr-site">
  ...
  <SectionWave className="kr-experienceWave"><div className="kr-waveInner"><div className="kr-sectionHead"><span className="kr-kicker">BİZİ BİZ YAPAN DENEYİM</span><h2>Mutlu Çocuklar, <strong>Güçlü Yarınlar</strong></h2><p>Yılların deneyimini çağdaş eğitim yaklaşımıyla birleştiriyor, çocuklarımızın geleceğine sağlam bir temel atıyoruz.</p></div><div className="kr-experienceGrid"><div className="kr-bigExperience"><strong>20+</strong><span>Yıllık Deneyim</span><small>Çocukların dünyasını anlayan, gelişimlerini destekleyen eğitim yaklaşımı.</small></div><div className="kr-miniStat"><b>150+</b><span>Mutlu Öğrenci</span></div><div className="kr-miniStat"><b>20+</b><span>Eğitici Atölye</span></div><div className="kr-miniStat"><b>5+</b><span>Servis Güzergahı</span></div></div></div></SectionWave>

  <SectionWave id="classes" className="kr-classesWave"><div className="kr-waveInner"><div className="kr-sectionHead"><span className="kr-kicker">EĞİTİM GRUPLARIMIZ</span><h2>Sınıflarımız</h2><p>Yaş ve gelişim dönemlerine uygun, keşfetmeyi teşvik eden sınıf ortamlarımız.</p></div><div className="kr-cardGrid">{classes.map((x,i)=><article className="kr-featureCard" key={x}><div className="kr-cardIcon">{['☀','★','♡','✿'][i]}</div><h3>{x}</h3><p>Oyun temelli öğrenme, sosyal gelişim ve bireysel keşif için tasarlanmış sınıfımız.</p><a href="#contact">Sınıfı İncele →</a></article>)}</div></div></SectionWave>
